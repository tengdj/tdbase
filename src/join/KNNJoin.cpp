...
inline void update_candidate_list_knn(candidate_entry* cand, query_context& ctx) {
    ...
    for (int i = 0; i < list_size && ctx.knn > cand->candidate_confirmed;) {
        ...
        if (global_ctx.verbose >= 1) {
            log("%ld\t%5ld sure closer %3d maybe closer %3d (%3d +%3d)", cand->mesh_wrapper->id, cand->candidates[i].mesh_wrapper->id, sure_closer,
                maybe_closer, cand->candidate_confirmed, cand_left);
        }
...
void SpatialJoin::nearest_neighbor(query_context ctx) {
    ...
    for (uint32_t lod : ctx.lods) {
        ...
        log("%ld polyhedron has %d candidates %d voxel pairs %.2f voxel pairs per candidate", candidates.size(), candidate_num, pair_num,
            (1.0 * pair_num) / candidates.size());
        ...
        for (candidate_entry* c : candidates) {
            ...
            for (candidate_info& ci : c->candidates) {
                ...
                if (ctx.use_aabb) {
                    ...
                    if (global_ctx.verbose >= 1) {
                        log("%ld\t%ld:\t[%.2f, %.2f]->[%.2f, %.2f]", wrapper1->id, wrapper2->id, ci.distance.mindist, ci.distance.maxdist,
                    ...
                    for (voxel_pair& vp : ci.voxel_pairs) {
                        ...
                        if (vp.v1->num_triangles > 0 && vp.v2->num_triangles > 0) {
                            ...
                            if (lod == ctx.highest_lod()) {
                                ...
                                dist.mindist =
                                    std::max(dist.mindist, res.distance - wrapper1->getHausdorffDistance() - wrapper2->getHausdorffDistance());
                                dist.maxdist = std::min(dist.maxdist,
                                                        res.distance + wrapper1->getProxyHausdorffDistance() + wrapper2->getProxyHausdorffDistance());
                                //								dist.maxdist = std::min(dist.maxdist, res.distance);
                            ...
                            if (global_ctx.verbose >= 1) {
                                log("%ld(%d)\t%ld(%d):\t[%.2f, %.2f]->[%.2f, %.2f] res: [%.2f, %.2f, %.2f]", wrapper1->id, res.p1, wrapper2->id,
                                    res.p2, vp.dist.mindist, vp.dist.maxdist, dist.mindist, dist.maxdist, res.min_dist, res.distance, res.max_dist);
                            }
                            vp.dist = dist;
