...
bool parse_OFF_sign(const char* data, size_t& offset, size_t len) {
    ...
    while (offset < len - 3) {
        if ((data[offset] == 'O' || data[offset] == 'o') && (data[offset + 1] == 'F' || data[offset + 1] == 'f') &&
...
bool TMesh::parse(const char* data, size_t len) {
    ...
    if (!parse_int(data, offset, len, num_vertices) || !parse_int(data, offset, len, num_faces) || !parse_int(data, offset, len, num_edges)) {
        return false;
    }
    if (num_vertices == 0) {
        return true;
