...
bool HiMesh::isRemovable(Vertex_handle v) const {
    ...
    //		return false;
    //	}
    if (v != vh_departureConquest[0] && v != vh_departureConquest[1] && !v->isConquered() && v->vertex_degree() > 2 && v->vertex_degree() <= 8) {
        // test convexity
...
void HiMesh::encodeBaseMesh() {
    ...
    for (HiMesh::Facet_iterator fit = facets_begin(); fit != facets_end(); ++fit) {
