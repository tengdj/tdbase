...
aab::aab(float min_x, float min_y, float min_z, float max_x, float max_y, float max_z) {
    low[0] = min_x;
    low[1] = min_y;
    low[2] = min_z;
    high[0] = max_x;
...
bool aab::intersect(aab& object) {
    return !(object.low[0] >= high[0] || object.high[0] <= low[0] || object.low[1] >= high[1] || object.high[1] <= low[1] ||
