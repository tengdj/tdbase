...
// compression mode
HiMesh::HiMesh(string& str, bool completeop) : CGAL::Polyhedron_3<MyKernel, MyItems>() {
...
HiMesh::HiMesh(char* data, size_t dsize, bool owndata) : CGAL::Polyhedron_3<MyKernel, MyItems>() {
    assert(dsize > 0);
    srand(PPMC_RANDOM_CONSTANT);
...
aab HiMesh::shrink(float shrink) {
    ...
    for (Vertex_iterator vi = vertices_begin(); vi != vertices_end(); vi++) {
        Point p = vi->point();
        vi->point() = Point((p[0] - mean_loc[0]) / shrink + mean_loc[0], (p[1] - mean_loc[1]) / shrink + mean_loc[1],
