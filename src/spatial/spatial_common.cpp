...
Polyhedron* make_cube(aab box) {
    ...
    Polyhedron::Halfedge_handle h = P->make_tetrahedron(Point(box.high[0], box.low[1], box.low[2]), Point(box.low[0], box.low[1], box.high[2]),
                                                        Point(box.low[0], box.low[1], box.low[2]), Point(box.low[0], box.high[1], box.low[2]));
    ...
    Polyhedron::Halfedge_handle f = P->split_facet(g->next(), g->next()->next()->next());
...
Polyhedron* make_cubes(vector<Voxel*>& boxes) {
    ...
    for (Voxel* b : boxes) {
        Polyhedron::Halfedge_handle h = P->make_tetrahedron(Point(b->high[0], b->low[1], b->low[2]), Point(b->low[0], b->low[1], b->high[2]),
                                                            Point(b->low[0], b->low[1], b->low[2]), Point(b->low[0], b->high[1], b->low[2]));
        Polyhedron::Halfedge_handle g = h->next()->opposite()->next();
        P->split_edge(h->next());
        P->split_edge(g->next());
        P->split_edge(g);
