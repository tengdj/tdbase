...
map<float, HiMesh::Face_iterator> HiMesh::encode_facets() {
    ...
    for (Facet_iterator f = facets_begin(); f != facets_end(); ++f) {
        ...
        do {
            Triangle t(e1->vertex()->point(), e2->vertex()->point(), e2->next()->vertex()->point());
...
pair<float, float> HiMesh::computeHausdorfDistance(HiMesh* original_mesh) {
    ...
    if (global_ctx.verbose >= 3) {
        if (i_curDecimationId % 2 != 0) {
            log("step: %2d smp: %.3f h_cal: %.3f ph_cal:%.3f avg_hdist#vertices: %ld #facets: %ld", i_curDecimationId, smp_tm, caldist_tm,
                ph_caldist_tm, size_of_vertices(), size_of_triangles());
            log("encode %d:\t[%.2f %.2f]\t%ld", i_curDecimationId, current_hausdorf.first, current_hausdorf.second, size_of_vertices());

            printf("%2d\t%.3f\t%.3f\t%.3f\n", i_curDecimationId, caldist_tm, ph_caldist_tm, caldist_tm + ph_caldist_tm);

            printf("%.3f\t%.3f\t", caldist_tm, ph_caldist_tm);
        }
    }
