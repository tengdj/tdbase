...
static void profile_decoding(int argc, char** argv) {
    ...
    logt("compress to %d vertices %d edges %d faces", sst, compressed->size_of_vertices(), compressed->size_of_halfedges() / 2,
         compressed->size_of_triangles());
    ...
    for (int i = start_lod; i <= end_lod; i++) {
        ...
        logt("decompress %3d lod %5d vertices %5d edges %5d faces avg(%.4f)", starttime, lod, tested[0]->size_of_vertices(),
             tested[0]->size_of_halfedges() / 2, tested[0]->size_of_triangles(), testedtime / itertime);
