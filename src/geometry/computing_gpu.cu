 ...
 *
 * */
__device__ inline void SegPoints(float VEC[3],
                                 float X[3],
 ...
 * almost all cases
 * */
__device__ inline float TriDist_seg(const float* S, const float* T, bool& shown_disjoint, bool& closest_find) {
    // closest points
    float P[3];
    float Q[3];
 ...
 *
 * */
__device__ inline float TriDist_other(const float* S, const float* T, bool& shown_disjoint) {
    // closest points
    float P[3];
    float Q[3];
...
__global__ void TriDist_cuda(const float* data,
                             const uint32_t* offset_size,
                             const float* hausdorff,
                             result_container* dist,
                             uint32_t tri_offset_1,
                             uint32_t tri_offset_2_start) {
    // which batch
    ...
    //	printf("%d %d:\t%f %f | %d %d:\t%f %f | %f %f %f | %f %f\n", obj_offset1, tri_offset_1, phdist1, hdist1, obj_offset2, tri_offset_2, phdist2,
    //hdist2, low_dist, high_dist, dd, dist[pair_id].min_dist, dist[pair_id].max_dist);
}

__global__ void TriInt_cuda(const float* data,
                            const uint32_t* offset_size,
                            const float* hausdorff,
...
//__global__
// void TriInt_cuda(const float *data, const float *hausdorff, const uint32_t *offset_size, result_container *intersect, uint32_t cur_offset_1,
// uint32_t cur_offset_2_start){
//
 ...
 *
 * */
void MeshDist_batch_gpu(gpu_info* gpu,
                        const float* data,
                        const uint32_t* offset_size,
                        const float* hausdorff,
